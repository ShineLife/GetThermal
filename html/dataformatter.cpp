...
void DataFormatter::FindMinMax(const uvc_frame_t *input, QPoint &minPoint, uint16_t &minVal, QPoint &maxPoint, uint16_t &maxVal) const
{
    ...
    my_count++;
    if(my_count >= 20){
        float tem = (maxVal-27315)/100.0f;
        char TempString[16];
        snprintf(TempString, sizeof(TempString), "%f", tem);
        
        //my_times += 1;
        //if(my_times > 50)
        //    my_times = 1;
        //char timeString[2];
        //snprintf(timeString, sizeof(timeString), "%d", my_times);
        char buffer[256];
        strncpy(buffer, "curl http://127.0.0.1/temperature.php?val=", sizeof("curl http://127.0.0.1/temperature.php?val="));
        strncat(buffer, TempString, sizeof(TempString));
        system(buffer);
        //std::ofstream myfile("/var/test.txt");
        //myfile << TempString;
        //myfile.close();
        my_count = 0;
    }
